...
const StepThree: React.FC<StepThreeProps> = React.memo(({ dish, handleChange, back, handleSubmit }) => <>
    ...
    <div className="mb-2">
        ...
        <Input element="title" onChange={handleChange} required={true} text="Chicken Salad" />
    </div>
    <div className="mb-2">
    ...
    <div className="md:flex md:items-center">
      <div className="mb-2 md:mr-2">
        <Label text="Preperation Time (mins)" element="time_prep" />
        <div className="relative">
          <Input element="time_prep" onChange={handleChange} pattern="[0-9]*" text="0" />
        </div>
      </div>
      <div className="mb-2">
        <Label text="Cooking Time (mins)" element="time_cook" />
        <div className="relative">
          <Input element="time_cook" onChange={handleChange} pattern="[0-9]*" text="0" />
        </div>
      </div>
    </div>
  ...
  <button 
    ...
    disabled={dish.title === '' || dish.ingredients.length <= 0 || dish.methods.length === 0}>
    <span>Save</span></button> {(!dish.title || dish.ingredients.length <= 0 || dish.methods.length < 0) && 
      <em className="ml-2 text-sm text-red-700"><strong>1</strong> Ingredient, Method, and Title Required*</em> }
</>);
